...
-- ────────────────────────────────────────────────────────────
-- ÉTAPE 9 : MENU CHILL LOUNGE (bar-restaurant, cocktails, Macampagne)
INSERT INTO public.categories (nom, description, emoji, ordre, actif) VALUES
('Cocktails et Bar','Cocktails, vins, bieres, alcools','🍸',1,true),
('Entrees et Snacks','Leger a partager','🥗',2,true),
('Plats','Cuisine locale et internationale','🍽️',3,true),
('Desserts','Douceurs maison','🍰',4,true),
('Softs et Boissons','Jus, sodas, eau','🥤',5,true)
ON CONFLICT DO NOTHING;

INSERT INTO public.produits (nom, description, prix, categorie_id, disponible, ordre) VALUES
('Chill Cocktail Special','Recette signature Chill Lounge',7.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,1),
('Mojito','Rhum, menthe fraiche, citron vert, soda',7.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,2),
('Sex on the Beach','Vodka, jus de peche, jus d orange, grenadine',8.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,3),
('Pina Colada','Rhum, lait de coco, jus d ananas',8.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,4),
('Biere Primus 65cl',NULL,4.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,5),
('Vin Rouge (verre)',NULL,7.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,6),
('Whisky Maison','Blend selectionne, avec ou sans glace',9.00,(SELECT id FROM categories WHERE nom='Cocktails et Bar'),true,7),
('Salade d Avocat','Avocat, tomates, oignons, vinaigrette citron',7.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,1),
('Ailes de Poulet x6','Epicees, sauce dip',8.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,2),
('Samoussa x4','Farce viande epicee',5.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,3),
('Frittes Maison',NULL,3.00,(SELECT id FROM categories WHERE nom='Entrees et Snacks'),true,4),
('Poulet Braise et Riz','Demi-poulet braise, riz jaune, salade',13.00,(SELECT id FROM categories WHERE nom='Plats'),true,1),
('Tilapia Frit','Tilapia entier frit, piment, citron',12.00,(SELECT id FROM categories WHERE nom='Plats'),true,2),
('Pates Bolognaise','Spaghetti, sauce viande hachee maison',10.00,(SELECT id FROM categories WHERE nom='Plats'),true,3),
('Poulet Roti aux herbes',NULL,14.00,(SELECT id FROM categories WHERE nom='Plats'),true,4),
('Riz au Gras','Riz cuisines, legumes, epices',9.00,(SELECT id FROM categories WHERE nom='Plats'),true,5),
...
('Tiramisu Maison',NULL,7.00,(SELECT id FROM categories WHERE nom='Desserts'),true,3),
('Jus de Fruit Frais','Mangue, passion, ananas, bissap',3.50,(SELECT id FROM categories WHERE nom='Softs et Boissons'),true,1),
('Coca Cola 33cl',NULL,2.50,(SELECT id FROM categories WHERE nom='Softs et Boissons'),true,2),
('Eau Minerale 75cl',NULL,2.00,(SELECT id FROM categories WHERE nom='Softs et Boissons'),true,3)
ON CONFLICT DO NOTHING;
